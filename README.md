...
- **There is new library surface that no command reaches yet.** Fourteen
  ...
  - `termproof::evidence` also grew `collector`, the ordered step model those
    ...
    than inside it — join the two with `manifest.attach_to(&mut result)`, which
    adds one artifact entry, refuses a manifest from a different run, and
    leaves the result payload and every existing reader of it untouched. Every
    captured step gets its screen *text* on disk, not only its PNG: an image
    shows a human what happened, it does not show what an assertion matched
    against. Captures read through `ScreenSource`, a one-method view of
    `Session` with a blanket impl, so every backend is one for free and a
    replayed cast or a test fixture can be captured without pretending to own
    a process — and text, grid and raw log come back from a single call, so
    they cannot end up describing different instants of a running program.
